...
Open a draft or choose New design. Choose compatible body, hair, face, top, bottom and footwear parts and supported colours. Body proportions filter clothing options. All four directions show standing, animated walking and sitting. Turn off Animate walking to pause the animation. Save draft writes the recipe under `drafts/` with a stable ID independent of its 1–24 character name. Names are Player-facing in every Room, so saving and publishing both refuse names that read as a slur, including padded and digit-substituted spellings; the refused terms live in `SLURS_ANYWHERE`/`SLURS_AS_WORDS` at the top of `authoring.py`. Restart the editor and reopen it from the library. Duplicate creates a saved independent identity; rename/edit preserve identity. Deleting a draft leaves other drafts and published artifacts alone. Unsaved changes prompt before switching designs.

Check designs in the library to select them for publication. Use the arrows to arrange exactly ten saved designs, then Publish collection. Validation requires distinct IDs, names, six compatible parts, supported palettes and every source animation frame. Errors explain what needs fixing; the prior publication remains unchanged. Drafts may be incomplete on disk but cannot be previewed or published until complete.

Publication snapshots names, stable IDs, ordered standing/walking sheets and seated leg sheets into `frontend/src/published-avatars.json` using atomic file replacement. No draft recipe or source artwork is included. Both game builds consume this same file. It prepares the next release and never changes a live backend's collection. Build both components and restart the backend together; existing Rooms are cleared on restart.
...
5. Update `frontend/public/assets/avatars/CREDITS.csv`, `credits.html` and applicable license text. Reuse the same attribution for recolours. Initial ten designs use only the already credited layers. `legacy-recipes.json` preserves the original six recipes and `reference/` preserves their visual reference sheets.
...
The Player build includes the published collection and attribution only. Vite never imports this editor or drafts; Maven explicitly includes only the published JSON. Do not put authoring resources in `frontend/public/`.
