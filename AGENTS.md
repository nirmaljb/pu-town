...
- `frontend/src/network-inbox.ts` — queue of decoded server events, each stamped with its arrival time.
...
- `frontend/src/meeting-area.ts` — loads the PU Town (Tiny Swords) tilemap and every tileset it embeds, draws its tile layers and its y-sorted, animated objects, the Emergency button, and the Seat chairs shown in the Lobby and Meetings.
- `frontend/public/maps/pu-town/` — the served Tiled map, its tilesets, and `pu-town.collision.json`, the source of both obstacle lists. `pu-town/` at the root keeps the Tiled `.tmx` sources, the generator and previews; copy its `pu-town.json`, `pu-town.collision.json` and `tilesets/**/*.png` here after regenerating, then regenerate both obstacle lists.
...
- `backend/src/main/java/dev/lpa/pu_go/room/RoomRules.java` — authoritative town bounds, obstacles, capacity, and Seat positions and Facing; every Roam begins at the Seat.
...
- Town dimensions and obstacles are duplicated in backend `RoomRules.java` and frontend `room-rules.ts`, both copied from `frontend/public/maps/pu-town/pu-town.collision.json` (`RoomRulesTest` and `field-controller.test.mjs` fail if either drifts from it); Seat geometry is in `RoomRules.seatX/seatY` and `meetingSeat`; Vision per Role and ranges in `FieldRules.java` and `room-rules.ts`, and Seat Facing in `RoomRules.seatFacing` and `avatar-facing.ts`. Keep shared values synchronized; the backend remains authoritative.
...
- The Game clock is the server's. Phases end on their own deadline, each deadline derives from the one it replaces, and every submission carries the round it was chosen in. The client counts `remainingMs` down from when the message arrived (`WorldState.phaseEndsAt`), never from when a frame applied it, so a hidden tab stays in step.
