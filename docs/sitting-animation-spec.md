...
An arriving Avatar lowers into its assigned chair over 420 ms, then holds a seated idle pose. All ten published Avatar Presets retain their existing LPC head, hair and upper-body artwork; published bent-leg sheets use each recipe’s trouser and footwear palettes. Heads and torsos retain their original proportions. Four-direction Facing continues to point inward.
...
Automated coverage combines the real inbox/frame boundary with seating timing to check snapshot and arrival ordering, readiness, accepted appearance changes, repeated snapshots, Host updates, Start, Leave, Reconnect and delayed frames. Run the full frontend checks, then use two clients to check Join, Ready, Start and Leave visually. The artwork gallery at `/test/sitting-preview.html` displays all ten presets in every Facing for manual inspection.
