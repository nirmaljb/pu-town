...
This decision revises ADR 006's new-membership-on-reconnect policy. Issues #2–#6 implement independent heartbeat health, credential-based 120-second membership retention, same-tab refresh and atomic takeover, fifteen-second Host grace, and full recovery/Leave controls. Session storage keeps ordinary tabs independent; duplicated tabs intentionally follow takeover semantics. Close code 4001 tells a displaced client to stop recovery. The server determines expiry; local retry clocks only schedule attempts.
...
Automatic retries continue throughout the recovery window with increasing delays capped at five seconds, and run immediately on return from tab suspension. Failed attempts do not extend the server-owned reservation deadline. Expired recovery never silently creates a new membership: as revised by ADR 0009, show “Your place in the Room expired” and “Back to lobby selection”, which clears intent and returns to Create Room / Join Lobby without a fresh Join shortcut. Server restart clears recovery state and Rooms, and the interface explains that the Room is no longer available.

While any recoverable membership remains, an all-disconnected Room retains its phase. If the Host grace period has elapsed without a connected successor, the first returning Player becomes Host. ADR 0009 revises the final-membership rule: a started Room is removed when its final membership ends, while a never-started Room begins the existing five-minute empty-Lobby expiry.
