...
The Host may start with any nonempty group regardless of readiness for the current testing version. This is deliberately not a production start gate. The final departure resets phase to Lobby while retaining the existing empty-Room expiry. ADR 0008 supersedes the original fresh-membership-on-reconnect decision: recovery retains membership and Seat, follows current phase, and does not reclaim transferred Host authority.
